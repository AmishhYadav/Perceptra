...
phase: 2
plan: 1
title: "Behavioral Data Generator"
wave: 1
depends_on: []
files_modified:
...
# Plan 1: Behavioral Data Generator

## Goal
Implement a `BehavioralDataGenerator` class that produces realistic synthetic telemetry datasets using per-class Gaussian distributions with partial class overlap and additive noise.

## must_haves
- Generator uses exact feature means/variances from CONTEXT.md
- Partial overlap between distracted/confused classes
- Controllable noise level (default σ=0.08)
- All values clipped to [0, 1] range
- Seeded with numpy seed=42 by default (reproducible)

## Tasks

<task id="2.1.1">
<title>Implement BehavioralDataGenerator class</title>
<read_first>
- src/data/schemas.py
- .planning/phases/02-synthetic-data-generation/02-CONTEXT.md
</read_first>
<action>
...
Update `src/data/__init__.py` to export the generator:
```python
...
<acceptance_criteria>
- `src/data/generator.py` contains `class BehavioralDataGenerator`
- `src/data/generator.py` contains `CLASS_DISTRIBUTIONS` with all 3 classes
- `src/data/generator.py` contains `CLASS_PROPORTIONS` dict with values summing to 1.0
- `src/data/generator.py` contains `def generate(self)` returning tuple
- `src/data/generator.py` contains `def generate_split(self`
- `src/data/generator.py` contains `np.clip(samples, 0.0, 1.0)`
- `src/data/__init__.py` contains `from .generator import BehavioralDataGenerator`
</acceptance_criteria>
</task>
...
python -c "
from src.data import BehavioralDataGenerator
gen = BehavioralDataGenerator(n_samples=15000)
X, y = gen.generate()
print(f'X shape: {X.shape}, y shape: {y.shape}')
print(f'X range: [{X.min():.3f}, {X.max():.3f}]')
print(f'Class counts: focused={sum(y==0)}, distracted={sum(y==1)}, confused={sum(y==2)}')
assert X.min() >= 0.0 and X.max() <= 1.0
assert X.shape == (15000, 8)
